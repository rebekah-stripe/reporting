/***************************************************
Step 2. Get the weekly processing volume
Generate processing volume query 

****************************************************/
-- 1. delete last week backup
drop table usertables.last_week_weekly_processing_upsells;
-- 2. rename this week to last week
alter table usertables.current_weekly_processing_upsells rename to last_week_weekly_processing_upsells;
-- 3. create base processing table
create table usertables.current_weekly_processing_upsells as (
select
...
sales_funnel__activation_date as upsells.effective_activation_date,
case when datediff('d', upsells.effective_activation_date, capture_date) >= 0 and datediff('d', upsells.effective_activation_date, capture_date) < 91 then 1 else 0 end as ninety_day_live,
case when datediff('d', upsells.effective_activation_date, capture_date) >= 0 and datediff('d', upsells.effective_activation_date, capture_date) < 366 then 1 else 0 end as first_year_sold,
COALESCE(SUM(ap.npv_usd_fixed / 100), 0)*upsells.pct_share_of_npv AS npv_fixed_fx
...
JOIN usertables.current_upsells as upsells ON upsells.sales_merchant_id = ap.sales_merchant_id
where
